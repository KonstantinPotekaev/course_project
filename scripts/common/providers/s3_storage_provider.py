...
class S3StorageProvider:
    ...
    def __init__(self, host: str, access_key: str, secret_key: str):
        ...
        """
        self._logger = logging.getLogger("das_client.s3_provider")
        self._host = host
    ...
    def _get_s3_client(self) -> BaseClient:
        ...
        return boto3.client(
            "s3",
            endpoint_url=f"{self._host}:{self.PORT}",
    ...
    def get_object(self,
        ...
        try:
            ...
            self._logger.warning(f"S3 key not found: {bucket}:{s3_key}")
            return None
        ...
        with futures.ThreadPoolExecutor(max_workers=thread_count) as executor:
            ...
            for task in futures.as_completed(tasks_pool):
                try:
                    ...
                    self._logger.error(f"Error in load_objects: {ex}")
            return s3_objects
    ...
    def create_bucket(self, bucket: str):
        ...
        """
        self._logger.debug("Create s3 bucket: %s", bucket)
        try:
            ...
            self._logger.warning(f"Bucket '{bucket}' already exists.")
        except self._s3_client.exceptions.BucketAlreadyOwnedByYou:
            self._logger.warning(f"Bucket '{bucket}' already owned by you.")
        except ClientError as e:
            code = e.response["Error"]["Code"]
            raise ConnectionError(
                response_message(
    ...
    def delete_bucket(self, bucket: str):
        ...
        """
        self._logger.debug("Deleting all objects in S3 bucket: %s", bucket)
        try:
            ...
            if "Contents" in objects:
                ...
                self._logger.debug("All objects deleted from bucket: %s", bucket)

            self._logger.debug("Deleting bucket: %s", bucket)
